...
show_banner() {
    echo ""
    echo -e "${MAGENTA}╔════════════════════════════════════════════════════╗${NC}"
    echo -e "${MAGENTA}║        🌸 Cute Claude Hooks 安装程序 🌸             ║${NC}"
...
install_hooks() {
    ...
    if command -v curl &> /dev/null; then
        curl -fsSL "$SCRIPT_URL/tool-tips-post.sh" -o "$HOOKS_DIR/tool-tips-post.sh"
    elif command -v wget &> /dev/null; then
        wget -q "$SCRIPT_URL/tool-tips-post.sh" -O "$HOOKS_DIR/tool-tips-post.sh"
    else
...
install_localize() {
    ...
    for file in "${files[@]}"; do
        echo "📥 下载 $file..."
        if command -v curl &> /dev/null; then
            curl -fsSL "$SCRIPT_URL/localize/$file" -o "$LOCALIZE_DIR/$file"
        elif command -v wget &> /dev/null; then
            wget -q "$SCRIPT_URL/localize/$file" -O "$LOCALIZE_DIR/$file"
        fi

        if [ -f "$LOCALIZE_DIR/$file" ]; then
